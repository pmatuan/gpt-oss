...
void warm_up(Transformer *transformer, Tokenizer *tokenizer) {
  ...
  HIP_CHECK(hipMalloc(&gpu_activations.d_qkv, (D * (Hq + 2 * Hk)) * sizeof(float)));
  ...
  HIP_CHECK(hipMalloc(&gpu_activations.d_key_cache, L * S * KV * sizeof(float)));
  HIP_CHECK(hipMalloc(&gpu_activations.d_value_cache, L * S * KV * sizeof(float)));
  ...
  HIP_CHECK(hipMalloc(&gpu_activations.d_token2row, S * sizeof(int)));
  {
    ...
    for (int i = 0; i < S; ++i)
      h_token2row[i] = i;
    HIP_CHECK(hipMemcpy(gpu_activations.d_token2row, h_token2row, S * sizeof(int),
  ...
  HIP_CHECK(hipMemcpy(gpu_weights_fp32.d_rms_attn_w, w->rms_attn_w, L * H * sizeof(float),
                      hipMemcpyHostToDevice));

  HIP_CHECK(hipMalloc(&gpu_weights_fp32.d_rms_ffn_w, L * H * sizeof(float)));
  HIP_CHECK(hipMemcpy(gpu_weights_fp32.d_rms_ffn_w, w->rms_ffn_w, L * H * sizeof(float),
                      hipMemcpyHostToDevice));

  const int QKV_D = D * (Hq + 2 * Hk);
  HIP_CHECK(hipMalloc(&gpu_weights_fp32.d_b_qkv, L * QKV_D * sizeof(float)));
  HIP_CHECK(hipMemcpy(gpu_weights_fp32.d_b_qkv, w->b_qkv, L * QKV_D * sizeof(float),
                      hipMemcpyHostToDevice));

  HIP_CHECK(hipMalloc(&gpu_weights_fp32.d_b_o, L * H * sizeof(float)));
  HIP_CHECK(
      hipMemcpy(gpu_weights_fp32.d_b_o, w->b_o, L * H * sizeof(float), hipMemcpyHostToDevice));

  HIP_CHECK(hipMalloc(&gpu_weights_fp32.d_attn_sinks, L * Hq * sizeof(float)));
  HIP_CHECK(hipMemcpy(gpu_weights_fp32.d_attn_sinks, w->attn_sinks, L * Hq * sizeof(float),
                      hipMemcpyHostToDevice));

  HIP_CHECK(hipMalloc(&gpu_weights_fp32.d_w_router, L * H * E * sizeof(float)));
  HIP_CHECK(hipMemcpy(gpu_weights_fp32.d_w_router, w->w_router, L * H * E * sizeof(float),
                      hipMemcpyHostToDevice));

  HIP_CHECK(hipMalloc(&gpu_weights_fp32.d_b_router, L * E * sizeof(float)));
  HIP_CHECK(hipMemcpy(gpu_weights_fp32.d_b_router, w->b_router, L * E * sizeof(float),
                      hipMemcpyHostToDevice));

  HIP_CHECK(hipMalloc(&gpu_weights_fp32.d_rms_out_w, H * sizeof(float)));
  HIP_CHECK(hipMemcpy(gpu_weights_fp32.d_rms_out_w, w->rms_out_w, H * sizeof(float),
                      hipMemcpyHostToDevice));

  debug_print_gpu_memory("after small FP32 weights");

  // Expert biases FP32
  HIP_CHECK(hipMalloc(&gpu_expert_bias.g_b_mlp1, (size_t)L * E * (2 * IM) * sizeof(float)));
  HIP_CHECK(hipMemcpy(gpu_expert_bias.g_b_mlp1, w->b_mlp1,
                      (size_t)L * E * (2 * IM) * sizeof(float),
                      hipMemcpyHostToDevice));
  HIP_CHECK(hipMalloc(&gpu_expert_bias.g_b_mlp2, (size_t)L * E * H * sizeof(float)));
  HIP_CHECK(hipMemcpy(gpu_expert_bias.g_b_mlp2, w->b_mlp2, (size_t)L * E * H * sizeof(float),
                      hipMemcpyHostToDevice));
  ...
  HIP_CHECK(
      hipMalloc(&gpu_weights_bf16.d_token_embedding_table_bf16, (size_t)V * H * sizeof(bf16_t)));
  copy_fp32_to_bf16_device(w->token_embedding_table, (size_t)V * H,
                           gpu_weights_bf16.d_token_embedding_table_bf16, n_streams,
                           chunk_bytes);

  HIP_CHECK(hipMalloc(&gpu_weights_bf16.d_w_qkv_bf16, (size_t)L * QKV_D * H * sizeof(bf16_t)));
  copy_fp32_to_bf16_device(w->w_qkv, (size_t)L * QKV_D * H, gpu_weights_bf16.d_w_qkv_bf16,
                           n_streams, chunk_bytes);

  const int O_N = D * Hq;
  HIP_CHECK(hipMalloc(&gpu_weights_bf16.d_w_o_bf16, (size_t)L * H * O_N * sizeof(bf16_t)));
  copy_fp32_to_bf16_device(w->w_o, (size_t)L * H * O_N, gpu_weights_bf16.d_w_o_bf16, n_streams,
                           chunk_bytes);

  HIP_CHECK(
      hipMalloc(&gpu_weights_bf16.d_w_mlp1_bf16, (size_t)L * E * (2 * IM) * H * sizeof(bf16_t)));
  copy_fp32_to_bf16_device(w->w_mlp1, (size_t)L * E * (2 * IM) * H,
                           gpu_weights_bf16.d_w_mlp1_bf16, n_streams, chunk_bytes);

  HIP_CHECK(hipMalloc(&gpu_weights_bf16.d_w_mlp2_bf16, (size_t)L * E * H * IM * sizeof(bf16_t)));
  copy_fp32_to_bf16_device(w->w_mlp2, (size_t)L * E * H * IM, gpu_weights_bf16.d_w_mlp2_bf16,
                           n_streams, chunk_bytes);

  HIP_CHECK(hipMalloc(&gpu_weights_bf16.d_out_bf16, (size_t)V * H * sizeof(bf16_t)));
  copy_fp32_to_bf16_device(w->out, (size_t)V * H, gpu_weights_bf16.d_out_bf16, n_streams,
                           chunk_bytes);
...
float *gpu_forward(Transformer *transformer, int token, int pos) {
  ...
  PROFILE_KERNEL_LAUNCH("copy_embedding_bf16_row_kernel",
                        copy_embedding_bf16_row_kernel<<<gridH, block>>>(
                            gpu_activations.d_x, gpu_weights_bf16.d_token_embedding_table_bf16, token, H));

  for (int l = 0; l < p->n_layers; ++l) {
    // RMSNorm (attn)
    PROFILE_KERNEL_LAUNCH(
        "rmsnorm_kernel",
        rmsnorm_kernel<<<1, BLOCK_SIZE>>>(gpu_activations.d_t, gpu_activations.d_x, gpu_weights_fp32.d_rms_attn_w + l * H, H));
    ...
    PROFILE_KERNEL_LAUNCH(
        ...
        <<<gridQKV, block>>>(gpu_activations.d_qkv, gpu_activations.d_t, gpu_weights_bf16.d_w_qkv_bf16 + (size_t)l * QKV_D * H,
    ...
    PROFILE_KERNEL_LAUNCH(
        ...
        split_qkv_scatter_to_cache_kernel<<<gridQKV, block>>>(
            gpu_activations.d_q, gpu_activations.d_key_cache, gpu_activations.d_value_cache, gpu_activations.d_qkv, Hq, Hk, D, loff, pos * KV));

    dim3 gridApply(max(Hq, Hk));
    PROFILE_KERNEL_LAUNCH("fused_inline_rope_qkv_kernel",
                          fused_inline_rope_qkv_kernel<<<gridApply, D / 2>>>(
                              gpu_activations.d_q, gpu_activations.d_key_cache, pos, p->rope_theta,
    ...
    // --- Attention ---
    {
      ...
      PROFILE_KERNEL_LAUNCH(
          ...
          attention_kernel<<<grid, block, shmem_size>>>(
              ...
              gpu_activations.d_key_cache + loff,   // [S, KV] FP32 for this layer - no conversion overhead
              gpu_activations.d_value_cache + loff, // [S, KV] FP32 for this layer - no conversion overhead
              gpu_weights_fp32.d_attn_sinks, l, pos, D, Hq, Hk, S,
              (p->sliding_window > 0 && (l % 2 == 0)) ? gpu_activations.d_mask : nullptr));
    }
    ...
    PROFILE_KERNEL_LAUNCH("add_bias_residual_inplace_kernel",
                          add_bias_residual_inplace_kernel<<<gridO, block>>>(
                              gpu_activations.d_x, gpu_activations.d_tb2, gpu_weights_fp32.d_b_o + l * H, H));

    // FFN RMSNorm
    PROFILE_KERNEL_LAUNCH(
        "rmsnorm_kernel",
        rmsnorm_kernel<<<1, BLOCK_SIZE>>>(gpu_activations.d_t, gpu_activations.d_x, gpu_weights_fp32.d_rms_ffn_w + l * H, H));
    ...
    PROFILE_KERNEL_LAUNCH("add_bias_kernel_router",
                          add_bias_kernel<<<gridE, block>>>(
                              gpu_activations.d_router_score, gpu_weights_fp32.d_b_router + l * E, E));
    ...
    PROFILE_KERNEL_LAUNCH(
        ...
        topk_kernel_1token<<<1, BLOCK_SIZE, shared_mem_size>>>(
            gpu_activations.d_topk_v, gpu_activations.d_topk_i, gpu_activations.d_router_score, E, p->experts_per_token));
    PROFILE_KERNEL_LAUNCH(
        "softmax_kernel",
        softmax_kernel<<<1, 1>>>(gpu_activations.d_topk_v, p->experts_per_token));
    ...
    for (int kk = 0; kk < p->experts_per_token; ++kk) {
      ...
      PROFILE_KERNEL_LAUNCH("mlp1_fused_kernel", mlp1_fused_kernel<bf16_t>
                            <<<gridIM, block>>>(gpu_activations.d_gate_up, gpu_activations.d_t, gpu_weights_bf16.d_w_mlp1_bf16,
                                                gpu_expert_bias.g_b_mlp1, gpu_activations.d_topk_i, kk, l, E, H,
                                                IM, p->swiglu_limit));
      ...
      PROFILE_KERNEL_LAUNCH(
          ...
          <<<gridH, block>>>(gpu_activations.d_e_agg, gpu_activations.d_gate_up, gpu_weights_bf16.d_w_mlp2_bf16, gpu_expert_bias.g_b_mlp2,
                             gpu_activations.d_topk_i, gpu_activations.d_topk_v, kk, l, E, IM, H));
    }

    // Residual add (x += e_agg)
    PROFILE_KERNEL_LAUNCH(
        "residual_add_kernel",
        residual_add_kernel<<<gridH, block>>>(gpu_activations.d_x, gpu_activations.d_e_agg, H));
  }

  // Final RMSNorm
  PROFILE_KERNEL_LAUNCH("rmsnorm_kernel", rmsnorm_kernel<<<1, BLOCK_SIZE>>>(
                                              gpu_activations.d_t, gpu_activations.d_x, gpu_weights_fp32.d_rms_out_w, H));
  HIP_CHECK(hipMemcpy(gpu_activations.d_x, gpu_activations.d_t, H * sizeof(float), hipMemcpyDeviceToDevice));

  // LM head - Using MFMA optimized kernel
  const int V = p->vocab_size;
  dim3 gridV = get_gemv_grid_dim(V);
  PROFILE_KERNEL_LAUNCH("matmul_kernel", matmul_kernel<bf16_t>
                        <<<gridV, block>>>(gpu_activations.d_logits, gpu_activations.d_x, gpu_weights_bf16.d_out_bf16, H, V));
