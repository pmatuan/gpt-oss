...
static long long run_requests_on_device(Transformer *transformer,
  ...
  for (int batch_start = 0; batch_start < num_ctxs; batch_start += MAX_BATCH_SIZE) {
    ...
    while (num_finished < B) {
      ...
      int *d_next = gpu_forward_device_batch(transformer, h_tokens.data(),
                                             h_pos.data(), B, device_id,
                                             max_pos_in_batch);
      HIP_CHECK(hipMemcpy(h_next_tokens.data(), d_next,
                          (size_t)B * sizeof(int), hipMemcpyDeviceToHost));
