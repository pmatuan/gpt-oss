    ...
    const int *__restrict__ pos) // nullable; pos[b] < 0 -> skip row
{
  ...
  for (int k0 = 0; k0 < n; k0 += 16) {
    ...
    const int k_rem =
        max(0, min(4, n - k_base)); // 0..4 remaining in this slice
    ...
    acc00 = __builtin_amdgcn_mfma_f32_16x16x16bf16_1k(Avec_r0, Bvec_c0, acc00,
                                                      0, 0, 0);
    acc01 = __builtin_amdgcn_mfma_f32_16x16x16bf16_1k(Avec_r0, Bvec_c1, acc01,
                                                      0, 0, 0);
    acc10 = __builtin_amdgcn_mfma_f32_16x16x16bf16_1k(Avec_r1, Bvec_c0, acc10,
                                                      0, 0, 0);
    acc11 = __builtin_amdgcn_mfma_f32_16x16x16bf16_1k(Avec_r1, Bvec_c1, acc11,
                                                      0, 0, 0);
  }
...
__global__ void matmul_bias_gemm_kernel_float(
  ...
  for (int k_base = 0; k_base < n; k_base += TK) {
    ...
    for (int k = lane * K_STEP_MATMUL_FLOAT; k < vec_k;
         k += WF_SIZE * K_STEP_MATMUL_FLOAT) {
...
__global__ void count_expert_assignments_kernel(int *__restrict__ counts,
                                                const int *__restrict__ topk_i,
                                                const int *__restrict__ pos,
                                                int batch_size,
                                                int experts_per_token, int E) {
...
__global__ void build_expert_assignments_kernel(
    ...
    const int *__restrict__ expert_offsets, int *__restrict__ expert_counters,
    int *__restrict__ assignment_batches, int *__restrict__ assignment_slots,
    int batch_size, int experts_per_token, int E) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  const int total = batch_size * experts_per_token;
  if (idx >= total)
    return;
...
__global__ void mlp1_fused_gemm_kernel(
    ...
    const bf16_t *__restrict__ w_mlp1_all, const float *__restrict__ b_mlp1_all,
  ...
  const size_t base_2im =
      ((size_t)l_layer * (size_t)E + (size_t)expert_id) * (size_t)(2 * IM);
  const size_t gate_row_off = ((size_t)(2 * im_idx + 0)) * (size_t)H;
  const size_t up_row_off = ((size_t)(2 * im_idx + 1)) * (size_t)H;

  const bf16_t *__restrict__ w_gate =
      w_mlp1_all + (base_2im * (size_t)H) + gate_row_off;
  const bf16_t *__restrict__ w_up =
      w_mlp1_all + (base_2im * (size_t)H) + up_row_off;
...
#pragma unroll
  ...
  const uint16_t *__restrict__ w_gate_u16 =
      reinterpret_cast<const uint16_t *>(w_gate);
  const uint16_t *__restrict__ w_up_u16 =
      reinterpret_cast<const uint16_t *>(w_up);
...
__global__ void mlp2_bias_weighted_accum_gemm_kernel(
    ...
    const bf16_t *__restrict__ w_mlp2_all, const float *__restrict__ b_mlp2_all,
    const int *__restrict__ assignment_batches,
    const int *__restrict__ assignment_slots,
  ...
  const float *__restrict__ gate_vec =
      ...
      (((size_t)slot * (size_t)batch_size + (size_t)batch_idx) * (size_t)IM);
  const size_t base =
      ((size_t)l_layer * (size_t)E + (size_t)expert_id) * (size_t)H;
  const bf16_t *__restrict__ w_row =
      w_mlp2_all + (base + (size_t)h_idx) * (size_t)IM;
...
#pragma unroll
  ...
  const uint16_t *__restrict__ w_row_u16 =
      reinterpret_cast<const uint16_t *>(w_row);
