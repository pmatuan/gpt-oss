...
// ================= SPLIT MLP KERNELS FOR BETTER BATCH SCALING =================

// 1. MLP1 MatMul + Bias: t @ w_mlp1 + b_mlp1 -> [K,B,2*IM]
__global__ void mlp1_matmul_bias_kernel(
    float* __restrict__ out,                // [K, B, 2*IM] output
    const float* __restrict__ x,            // [B, H] input
    const bf16_t* __restrict__ w_mlp1_all,  // [L, E, 2*IM, H] weights
    const float* __restrict__ b_mlp1_all,   // [L, E, 2*IM] biases
    const int* __restrict__ topk_i,         // [B, K] expert indices
    ...
    int batch_size, const int *pos)
{
  ...
  const int output_dim = blockIdx.x * TM + wid;  // output dimension (0..2*IM-1)
  const int kidx = blockIdx.z;                   // expert index per token
  
  if (wid >= TM || output_dim >= 2*IM || kidx >= EXPERT_PER_TOKEN)
    return;

  // Load expert ID for current batch
  int expert_id = topk_i[(size_t)batch_idx * EXPERT_PER_TOKEN + kidx];
  ...
  for (int k_base = 0; k_base < H; k_base += TK) {
    ...
    // Load X for current batch into shared memory
    ...
    // Compute: weight[expert, output_dim, :] @ x[:]
    const size_t base = ((size_t)l_layer * (size_t)E + (size_t)expert_id) * (size_t)(2 * IM);
    const size_t weight_offset = ((base + (size_t)output_dim) * (size_t)H) + (size_t)k_base;
    const bf16_t* __restrict__ w_row = w_mlp1_all + weight_offset;
  ...
  if (lane == 0) {
    const size_t bias_offset = ((size_t)l_layer * (size_t)E + (size_t)expert_id) * (size_t)(2 * IM) + (size_t)output_dim;
    float result = acc_sum + b_mlp1_all[bias_offset];
    
    // Store result: out[kidx, batch_idx, output_dim]
    size_t out_offset = ((size_t)kidx * (size_t)batch_size + (size_t)batch_idx) * (size_t)(2 * IM) + (size_t)output_dim;
    out[out_offset] = result;
  }
}

// 2. MLP1 Split & SwiGLU: split gate/up and apply activation
__global__ void mlp1_split_swiglu_kernel(
    float* __restrict__ gate_up_out,        // [K, B, IM] output
    const float* __restrict__ gate_up_raw,  // [K, B, 2*IM] input
    float clip, int IM, int B, int K,
    const int *pos)
{
  const int batch_idx = blockIdx.y;
  ...
  const int dim_idx = blockIdx.x * BLOCK_SIZE + threadIdx.x;  // IM dimension
  const int kidx = blockIdx.z;                                // expert index
  
  if (dim_idx >= IM || kidx >= K) return;

  // Load gate and up values
  size_t base_offset = ((size_t)kidx * (size_t)B + (size_t)batch_idx) * (size_t)(2 * IM);
  float gate_val = gate_up_raw[base_offset + (size_t)(2 * dim_idx + 0)];
  float up_val   = gate_up_raw[base_offset + (size_t)(2 * dim_idx + 1)];

  // Apply clipping
  gate_val = __saturatef((gate_val + clip) / (2.0f * clip)) * (2.0f * clip) - clip;
  up_val   = __saturatef((up_val   + clip) / (2.0f * clip)) * (2.0f * clip) - clip;

  // Apply SwiGLU activation
  const float alpha = 1.702f;
  gate_val = gate_val * __saturatef(0.5f + 0.5f * tanhf(alpha * gate_val * 0.5f));
  gate_val = gate_val * (up_val + 1.0f);

  // Store result
  size_t out_offset = ((size_t)kidx * (size_t)B + (size_t)batch_idx) * (size_t)IM + (size_t)dim_idx;
  gate_up_out[out_offset] = gate_val;
}

// 3. MLP2 MatMul + Bias: gate_up @ w_mlp2 + b_mlp2 -> [K,B,H]
__global__ void mlp2_matmul_bias_kernel(
    float* __restrict__ out,                // [K, B, H] output
    const float* __restrict__ gate_up,      // [K, B, IM] input
    const bf16_t* __restrict__ w_mlp2_all,  // [L, E, H, IM] weights
    const float* __restrict__ b_mlp2_all,   // [L, E, H] biases
    const int* __restrict__ topk_i,         // [B, K] expert indices
    int l_layer, int E, int IM, int H, int B,
    const int *pos)
{
  const int batch_idx = blockIdx.y;
  if (batch_idx >= B) return;
  ...
  const int output_dim = blockIdx.x * TM + wid;  // output dimension (0..H-1)
  const int kidx = blockIdx.z;                   // expert index per token
  
  if (wid >= TM || output_dim >= H || kidx >= EXPERT_PER_TOKEN)
    return;
  ...
  // K loop over IM dimension
  for (int k_base = 0; k_base < IM; k_base += TK) {
    const int k_size = min(TK, IM - k_base);

    // Load gate_up for current batch/expert into shared memory
    const float* __restrict__ xb = gate_up + 
        (((size_t)kidx * (size_t)B + (size_t)batch_idx) * (size_t)IM + (size_t)k_base);
    ...
    // Compute: weight[expert, output_dim, :] @ gate_up[:]
    const size_t base = ((size_t)l_layer * (size_t)E + (size_t)expert_id) * (size_t)H * (size_t)IM;
    const bf16_t* __restrict__ w_row = w_mlp2_all + base + (size_t)output_dim * (size_t)IM + (size_t)k_base;
  ...
  float acc_sum = warp_reduce_sum(acc);
  if (lane == 0) {
    const size_t bias_offset = ((size_t)l_layer * (size_t)E + (size_t)expert_id) * (size_t)H + (size_t)output_dim;
    float result = acc_sum + b_mlp2_all[bias_offset];
    
    // Store result: out[kidx, batch_idx, output_dim]
    size_t out_offset = ((size_t)kidx * (size_t)B + (size_t)batch_idx) * (size_t)H + (size_t)output_dim;
    out[out_offset] = result;
  }
}

// 4. MLP2 Weighted Accumulation: sum(topk_v[k] * mlp2_out[k]) -> [B,H]
__global__ void mlp2_weighted_accum_kernel(
    float* __restrict__ e_agg,              // [B, H] accumulator
    const float* __restrict__ mlp2_out,     // [K, B, H] input
    const float* __restrict__ topk_v,       // [B, K] expert weights
    int H, int B, int K, const int *pos)
{
  ...
  const int dim_idx = blockIdx.x * BLOCK_SIZE + threadIdx.x;  // H dimension
  if (dim_idx >= H) return;

  float sum = 0.0f;
  
  // Sum over all experts for this batch/dimension
  for (int kidx = 0; kidx < K; kidx++) {
    float expert_weight = topk_v[(size_t)batch_idx * K + kidx];
    if (expert_weight != 0.0f) {
      float expert_out = mlp2_out[((size_t)kidx * (size_t)B + (size_t)batch_idx) * (size_t)H + (size_t)dim_idx];
      sum += expert_weight * expert_out;
    }
  }
