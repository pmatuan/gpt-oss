...
// Batched decode-time attention kernel. grid = (Hq, B), block = (WF_SIZE)
__launch_bounds__(64, 8)
__global__ void attention_kernel_batched(
    ...
    int layer_idx, const int* __restrict__ pos, int D, int Hq, int Hk, int S, int L,
    const float* __restrict__ mask, int batch
) {
  const int head = blockIdx.x;
  const int b = blockIdx.y;
  const int lane = threadIdx.x;

  if (b >= batch || head >= Hq) return;
  int pos_b = pos[b];
  if (pos_b < 0) return;
  ...
  // per-sample pointers
  const float* q_b = q + (size_t)b * Hq * D;
  const float* k_base = k_cache + ((size_t)b * L + (size_t)layer_idx) * (size_t)S * (size_t)kv_dim;
  const float* v_base = v_cache + ((size_t)b * L + (size_t)layer_idx) * (size_t)S * (size_t)kv_dim;
  float* out_b = out_tb + (size_t)b * Hq * D;

  extern __shared__ float s_att[];

  for (int t = lane; t <= pos_b; t += WF_SIZE) {
    const float* k_ptr = k_base + t * kv_dim + kv_head * D;
    ...
    for (int d = D4 << 2; d < D; ++d) score = fmaf(q_ptr[d], k_ptr[d], score);
    score *= rsqrt_D;
  ...
  float sum = 0.0f;
  for (int i = lane; i < att_size; i += WF_SIZE) { float v = expf(s_att[i] - maxv); s_att[i] = v; sum += v; }
  #pragma unroll
  ...
  if (lane < threads_per_dim) {
    ...
    const int start_dim = lane * dims_per_thread;
    const int end_dim = min(start_dim + dims_per_thread, D);
    for (int d0 = start_dim; d0 < end_dim; ++d0) {
      float result = 0.0f;
      for (int t = 0; t <= pos_b; ++t) {
        const float* v_ptr = v_base + t * kv_dim + kv_head * D;
        result = fmaf(s_att[t], v_ptr[d0], result);
      }
      out_b[head * D + d0] = result;
    }
  }
}
