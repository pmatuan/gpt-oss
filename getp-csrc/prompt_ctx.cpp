...
struct PromptCtx {
  ...
  PromptCtx()
      : idx(0), input_seq(""), prompt_tokens(nullptr),
        num_prompt_tokens(0), output_tokens(nullptr), output_str(),
        pos(0), token(0), finished(false), max_steps(0), h_logits(nullptr),
        logits_size(0), sampler(nullptr), num_generated(0), start_time(0.0),
        end_time(0.0), is_context_phase(true), user_data(nullptr) {}
};
