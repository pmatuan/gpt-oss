...
// Batched RMSNorm: one block per sample
__global__ void rmsnorm_kernel_batched(float *o, const float *x, const float *weight, int size) {
  const int b = blockIdx.x;
  const int tid = threadIdx.x;
  const int lane = tid & (WF_SIZE - 1);
  const int wid = tid >> 6;

  const float *xb = x + (size_t)b * size;
  float *ob = o + (size_t)b * size;
...
// Batched variant: grid.y = batch
__global__ void split_qkv_scatter_to_cache_kernel_batched(float *q, float *key_cache, float *value_cache,
                                                          const float *qkv, int n_attn_heads, int n_kv_heads,
                                                          int head_dim, int l, int S, int KV, int L,
                                                          const int *pos, int batch) {
  int b = blockIdx.y;
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (b >= batch) return;
  int pos_b = pos[b];
  if (pos_b < 0) return; // inactive
  ...
  size_t q_off = (size_t)b * q_size;
  size_t qkv_off = (size_t)b * total;
  size_t layer_base = ((size_t)b * L + (size_t)l) * (size_t)S * (size_t)KV;
  size_t pos_off = (size_t)pos_b * (size_t)KV;

  if (idx < q_size) {
    q[q_off + idx] = qkv[qkv_off + idx];
  } else if (idx < q_size + kv_size) {
    int k_idx = idx - q_size;
    key_cache[layer_base + pos_off + k_idx] = qkv[qkv_off + idx];
  } else {
    int v_idx = idx - q_size - kv_size;
    value_cache[layer_base + pos_off + v_idx] = qkv[qkv_off + idx];
  }
}
...
// Batched RoPE: grid = (max(Hq,Hk), batch), block = D/2
__global__ void fused_inline_rope_qkv_kernel_batched(float *q, float *key_cache, const int *pos,
                                                     float rope_theta, int n_q_heads, int n_k_heads, int head_dim,
                                                     float scaling_factor, float initial_context_length,
                                                     int l, int S, int KV, int L, int batch) {
  int b = blockIdx.y;
  int h = blockIdx.x;
  int i = threadIdx.x;
  if (b >= batch) return;
  ...
  // Q
  if (h < n_q_heads) {
    float *q_b = q + (size_t)b * (size_t)n_q_heads * head_dim;
  ...
  if (h < n_k_heads) {
    size_t layer_base = ((size_t)b * L + (size_t)l) * (size_t)S * (size_t)KV;
    size_t pos_off = (size_t)pos_b * (size_t)KV;
    int cache_idx = layer_base + pos_off + h * head_dim;
    float x1 = key_cache[cache_idx + i];
    float x2 = key_cache[cache_idx + half + i];
    key_cache[cache_idx + i] = x1 * c - x2 * s;
    key_cache[cache_idx + half + i] = x2 * c + x1 * s;
...
// Batched residual add over grid.y
__global__ void residual_add_kernel_batched(float *x, const float *residual, int size, int batch) {
  int b = blockIdx.y;
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (b >= batch || i >= size) return;
...
// Batched variant over grid.y
template <typename T>
__launch_bounds__(BLOCK_SIZE, 1) __global__
void fused_rmsnorm_matmul_bias_kernel_batched(float *__restrict__ y,
                                              const float *__restrict__ x,
                                              const T *__restrict__ w,
                                              const float *__restrict__ b,
                                              const float *__restrict__ rms_w,
...
template <typename T>
__launch_bounds__(BLOCK_SIZE, 1) __global__
void fused_matmul_bias_residual_kernel_batched(float *__restrict__ x,
                                               const float *__restrict__ y,
                                               const T *__restrict__ w,
                                               const float *__restrict__ b,
                                               int n, int d, int batch) {
  ...
  float *xb = x + (size_t)bidx * d;
  const float *yb = y + (size_t)bidx * n;
  ...
  if (lane == 0) xb[row] = xb[row] + (acc_all + b[row]);
}
...
// Batched final head (no bias)
template <typename T>
__launch_bounds__(BLOCK_SIZE, 1) __global__
void fused_rmsnorm_matmul_kernel_batched(float *__restrict__ y,
                                         const float *__restrict__ x,
                                         const T *__restrict__ w,
                                         const float *__restrict__ rms_w,
                                         int n, int d, int batch) {
...
#pragma unroll
  ...
  if (lane == 0) yb[row] = acc_all; // no bias
}
...
// Batched fused topk + softmax: one block per sample (shared memory size = E)
__global__ void fused_topk_softmax_kernel_batched(float *topk_values, int *topk_indices,
                                                  float *router_score, int num_experts, int experts_per_token) {
  int b = blockIdx.x;
  extern __shared__ float smem[];
  float *scores = smem;
  ...
  float *router_b = router_score + (size_t)b * num_experts;
  float *topk_v_b = topk_values + (size_t)b * experts_per_token;
  int *topk_i_b = topk_indices + (size_t)b * experts_per_token;

  for (int i = tid; i < num_experts; i += blockDim.x) scores[i] = router_b[i];
  __syncthreads();
  ...
  for (int k = 0; k < experts_per_token; k++) {
    float local_best = -INFINITY; int local_idx = -1;
    for (int i = tid; i < num_experts; i += blockDim.x) {
      float v = scores[i];
...
#pragma unroll
    ...
    if (tid % WF_SIZE == 0) { warp_vals[tid / WF_SIZE] = local_best; warp_idxs[tid / WF_SIZE] = local_idx; }
    __syncthreads();
    if (tid < WF_SIZE) {
      float bVal = (tid < BLOCK_SIZE / WF_SIZE) ? warp_vals[tid] : -INFINITY;
      int bIdx = (tid < BLOCK_SIZE / WF_SIZE) ? warp_idxs[tid] : -1;
#pragma unroll
      for (int off = WF_SIZE >> 1; off > 0; off >>= 1) {
        float oVal = __shfl_down(bVal, off, WF_SIZE);
        int oIdx = __shfl_down(bIdx, off, WF_SIZE);
      ...
      if (tid == 0) {
        topk_v_b[k] = bVal; topk_i_b[k] = bIdx; if (bIdx >= 0) scores[bIdx] = -INFINITY;
      }
    }
    __syncthreads();
  }

  // Softmax over selected top-k entries
  if (tid == 0) {
    float max_val = topk_v_b[0];
    for (int i = 1; i < experts_per_token; i++) max_val = fmax(max_val, topk_v_b[i]);
    float sum = 0.0f;
    for (int i = 0; i < experts_per_token; i++) { float ev = expf(topk_v_b[i] - max_val); topk_v_b[i] = ev; sum += ev; }
    float inv = 1.0f / sum; for (int i = 0; i < experts_per_token; i++) topk_v_b[i] *= inv;
  }
}
