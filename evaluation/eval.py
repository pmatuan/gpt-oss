...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["20b", "120b"],
        help=
        "Model type: 20b or 120b. If provided, sets default submission and references paths.",
    )
    parser.add_argument(
        "-s",
        "--submission",
...
def ensure_nltk_resources():
    ...
    for r in ["punkt", "wordnet", "omw-1.4"]:
        try:
            nltk.data.find(
                f"tokenizers/{r}") if r == "punkt" else nltk.data.find(
                    f"corpora/{r}")
...
def compute_meteor(refs: List[str], subm: List[str]) -> tuple[int, float]:
    ...
    print(f"computing METEOR in parallel with {max_workers} workers...",
          flush=True)
...
def compute_bertscore(refs: List[str], subm: List[str],
                      bs_model_type: str) -> tuple[int, float]:
    n = min(len(refs), len(subm))
    if n == 0:
        return 0, 0.0
    ...
    if device == "cuda":
        try:
            ...
            pass
        print(f"BERTScore device: GPU ({torch.cuda.device_count()} device(s))",
              flush=True)
    else:
    ...
    bs_batch = 64
    chunk_size = 128
    with tqdm(total=n, desc="BERTScore", dynamic_ncols=True,
              mininterval=0.2) as pbar:
        for i in range(0, n, chunk_size):
            j = min(i + chunk_size, n)
            _, _, F1 = scorer.score(subm[i:j],
                                    refs[i:j],
                                    batch_size=bs_batch,
                                    verbose=True)
...
def main():
    ...
    # Assert thresholds
    assert meteor >= thresholds[
        "meteor"], f"METEOR {meteor} < threshold {thresholds['meteor']}"
    assert bsf1 >= thresholds[
        "bertscore_f1"], f"BERTScore F1 {bsf1} < threshold {thresholds['bertscore_f1']}"
